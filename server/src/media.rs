...
/// Width, in pixels, of resized image used to generate perceptual ordinal in [perceptual_ordinal]
// Note: Any change to this value will probably require changes to the [image_ordinal] function since it is based
// on certain assumptions about this value and [ORDINAL_LENGTH].
const ORDINAL_WIDTH: usize = 4;

/// Height, in pixels, of resized image used to generate perceptual ordinal in [perceptual_ordinal]
// Note: Any change to this value will probably require changes to the [image_ordinal] function since it is based
// on certain assumptions about this value and [ORDINAL_LENGTH].
const ORDINAL_HEIGHT: usize = 4;

/// Number of 8-bit color channels used when calculating perceptual ordinals
const ORDINAL_CHANNEL_COUNT: usize = 3;

/// Length, in bytes, of a perceptual ordinal generated by [perceptual_ordinal]
...
pub struct Item {
    ...
    /// The perceptual ordinal for this media item
    ///
    /// See also [perceptual_ordinal].
    pub ordinal: Ordinal,
    ...
    /// The index (i.e. quality ranking) of this item within its duplicate group, if any
...
/// Calculate the perceptual ordinal for the [ORDINAL_WIDTH]x[ORDINAL_HEIGHT] image, `iter`.
///
/// We calculate the perceptual ordinal by rearranging the bits of the image such that the most significant bits of
/// each color channel of each pixel come first, followed by the next most significant bits, etc., with the least
/// significant bits last.
///
/// When the resulting value is interpreted as an 8*[ORDINAL_LENGTH]-bit big-endian number, it can be used to
/// quickly compare two images for similarity: if the absolute difference between the ordinals of each image is
/// within some epsilon value, then they are considered similar; otherwise, they are not.
///
/// Once we've calculated the ordinal for each image in a collection, we can sort the collection by their ordinals,
/// and then do a linear scan through the sorted collection to group images by similarity.  Finally, each of the
/// resulting groups may be passed to [deduplicate] to determine which, if any, are duplicates of others in that
/// group.
...
/// Perceptual ordinal for a media item as calculated by [perceptual_ordinal]
...
pub struct Ordinal {
    ...
    ///
    /// See also the [image_ordinal] function.
    pub image_ordinal: Vec<u8>,
}

impl Ordinal {
    /// Return `true` iff `self` is considered similar to `other`, i.e. if their `video_length_seconds` fields
    /// match and the absolute difference between their `image_ordinal` fields, when interpreted as a
    /// 8*[ORDINAL_LENGTH]-bit big-endian number, is less than a certain epsilon value.
    pub fn is_similar_to(&self, other: &Ordinal) -> bool {
        if self.video_length_seconds == other.video_length_seconds {
            ...
            // Define epsilon to correspond to an average absolute difference per channel of 2^5:
            let epsilon = BigUint::from_bytes_be(
                &[255u8; 5 * ((ORDINAL_WIDTH * ORDINAL_HEIGHT) / 8) * ORDINAL_CHANNEL_COUNT],
            );
...
///
/// The resulting value may be used to sort and group items by similarity prior to deduplicating them using
/// [deduplicate].
///
/// See [image_ordinal] for more details.
